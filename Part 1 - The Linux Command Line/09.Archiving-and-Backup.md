...
* *.tar* - 
* *.gz* - 
  * *.tgz, .tar.gz* - ```tar``` + ```gunzip``` utilizing the DEFLATE algorithm (a combination of LZ77 and Huffman coding)
* *.bz2* - 
  * *.tbz, .tar.bz2* - ```tar``` + ```bzip2``` utilizing the Burrows–Wheeler algorithm
* *.xz* - 
  * *.txz, .tar.xz* - ```tar``` + ```xz``` utilizing the LZMA2 algorithm
* *.zip* - *zip* - permits several algorithms though DEFLATE is the most common
* *.br* -
  * *.tbr, .tar.br* - ```tar``` + ```br``` utilizing LZ77 algorithm, Huffman coding and 2nd order context modeling
* *.zst* - 
  * *.tzst, .tar.zst* - ```tar``` + ```zstd``` utilizing LZ77 algorithm, Huffman coding and FSE
