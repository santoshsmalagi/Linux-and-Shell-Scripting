...
### Archiving (tar)
Archiving is the process of gathering up many files and bundling them into a single large file, in Linux based systems ```tar``` is the standard utility for archiving files. 
